package com.cty.hadoop.security;
...
public class HadoopSecurityUtil {
	...
	private static final String LOGIN_FAILED_CAUSE_AES256_WRONG =
		"(aes256 not support) aes256 not support by default jdk/jre, need copy local_policy.jar and US_export_policy.jar from remote server in path /opt/jdkxxx/jre/lib/security";
	...
	public static void setNameNodePrincipal(String nnPrincipalKey, String nnServerPrincipal) throws IOException {
		System.setProperty(nnPrincipalKey, nnServerPrincipal);
		String ret = System.getProperty(nnPrincipalKey);
		if (ret == null) {
			LOG.error(nnPrincipalKey + " is null.");
			throw new IOException(nnPrincipalKey + " is null.");
		}
		if (!ret.equals(nnServerPrincipal)) {
			LOG.error(nnPrincipalKey + " is " + ret + " is not " + nnServerPrincipal + ".");
			throw new IOException(nnPrincipalKey + " is " + ret + " is not " + nnServerPrincipal + ".");
		}
	}
